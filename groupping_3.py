...
class window(QtWidgets.QDialog):
    ...
    def init_UI(self) :
        ...
        self.make_model = make_model(self)
        ...
        self.ui.btn_start_modeling.clicked.connect(self.make_model.start)
...
class make_model(QThread) :
    def __init__ (self, parent) :
        ...
        self.db_daesin = pymysql.connect(host='221.148.138.91', port=3306, user='remote', password='1234', db='dbstock')
        self.cursor_daesin = self.db_daesin.cursor()
        self.cursor_daesin.execute('use dbstock;')

        self.db_kiwoom = pymysql.connect(host='61.77.150.183', port=3306, user='a1_user',password='4560',db='kiwoom',charset='utf8')
        self.cursor_kiwoom = self.db_kiwoom.cursor()
        self.cursor_kiwoom.execute('use kiwoom;')

        self.timesteps =  self.parent.ui.timesteps.value()
        self.offset =  self.parent.ui.offsets.value()
    ...
    def daesin_main(self, code, date):    
        ...
        df_dae = pd.read_sql(sql, self.db_daesin)
    ...
    def daesin_kospi(self, date):  
        ...
        df_kospi = pd.read_sql(sql_kospi, self.db_daesin)
    ...
    def daesin_issued(self, code):
        ...
        df_issued = pd.read_sql(sql_issued, self.db_daesin)
    ...
    def kiwoom(self, code, date) : 
        ...
        df_kiwoom = pd.read_sql(sql_kiwoom, self.db_kiwoom)
    ...
    def merge_df(self, date):
        
        df_dae, df_kiwoom, df_kospi= self.daesin_main(code, date,db=self.db_daesin),self.kiwoom(code, date,db=self.db_kiwoom), globals()['df_kospi_{}'.format(date.replace('-',''))]
    ...
    #     merged_df.drop('time_section',axis=1,inplace=True)
        ...
        merged_df['target'] = merged_df['종가'].pct_change(periods=self.offset)
    ...
    def split_sequence(self, timesteps, offset):
        
        year_ = self.parent.ui.cmb_year.currentText()
        mon_ = self.parent.ui.cmb_mon.currentText()
        day_ = self.parent.ui.cmb_date.currentText()
        date = year_+'-'+mon_.zfill(2)+'-'+day_.zfill(2)
    ...
    def concat_train_test(self, code_list):
        global code
        ...
        for i, code in (enumerate(code_list)) :
            ...
            try :
                ...
                if i == 0: 
                    x_train0, y_train0, x_test0, y_test0 = self.train_test_spilt()

                else :
                    x_train, y_train, x_test, y_test = self.train_test_spilt()
    ...
    #     dataset_load=np.load(path+'final_{}S_dataset_{}_{}.npz'.format(group,timesteps,offset))
    ...
    def run(self) :
        print('make_model start')
        global df_merged_
        print('make model : ',df_merged_)
        # x_train,x_test,y_train,y_test=self.load_final_data(path = './numpy/concat/',group = 'D',offset=3, timesteps=60) 
        ...
        # model = Sequential()
        # model.add(Dense(10, activation='relu',input_dim=x_train.shape[1],kernel_initializer='he_normal'))
